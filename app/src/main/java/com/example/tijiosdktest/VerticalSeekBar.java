...
public class VerticalSeekBar extends AppCompatSeekBar {
    ...
    private void caculateScale(MotionEvent event) {
        float y = event.getRawY();
        int[] outLocation = new int[2];
        getLocationOnScreen(outLocation);
//        float top = outLocation[1] - getY();
        mScale = (outLocation[1] - y) / getY();
//        Log.i("zzzzzzzzzzzzzzz",mScale + "");
    }
...
//        final float scale;
        ...
        if (value < 0 || available == 0) {
            ...
            mScale = value / (float) available;
        }

        final int max = getMax();
        final float progress = mScale * max;
